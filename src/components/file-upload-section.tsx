...
import { Activity, FolderSync, Search, X } from "lucide-react"
...
interface ProcessingState {
  ...
  message: string
  progress: number
  stage: "reading" | "parsing" | "indexing" | "saving"
}
...
export default function FileUploadSection({ processing, handleFileUpload, studentsCount }: FileUploadSectionProps) {
  ...
  if (!showNotice) {
    return <MainContent processing={processing} handleFileUpload={handleFileUpload} studentsCount={studentsCount} />
  }

  return (
    <div className="space-y-4">
      {/* التنويه المهم */}
      <div className="relative bg-gradient-to-r from-gray-900/20 to-gray-950/20 border border-amber-400/30 rounded-2xl p-6 backdrop-blur-sm">
        <button
      ...
      <MainContent processing={processing} handleFileUpload={handleFileUpload} studentsCount={studentsCount} />
    </div>
  )
}

function MainContent({ processing, handleFileUpload, studentsCount }: Omit<FileUploadSectionProps, 'user'>) {
  return (
