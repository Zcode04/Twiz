  ...
  return (
    <div className="space-y-6">
      ...
      {searchQuery.trim() && !selectedStudent && searchResults.length > 0 && (
        <div className="mt-4 max-h-20 overflow-y-auto bg-gray-900/5 rounded-xl shadow-lg border border-white/20">
          {searchResults.map((student) => (
            ...
            >
              <div className="flex-1">
                <p className="font-medium text-green-400">{highlightText(student.NOM, searchQuery)}</p>
                <p className="text-sm text-green-400">{student.Ecole}</p>
              </div>
      ...
      {searchQuery.trim() && searchResults.length === 0 && !selectedStudent && (
        <div className="mt-4 p-4 bg-red-500/20 border border-red-500/30 rounded-full">
